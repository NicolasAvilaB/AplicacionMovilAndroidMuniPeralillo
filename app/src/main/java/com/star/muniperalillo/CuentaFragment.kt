...
class CuentaFragment : Fragment(), DialogoConfirmacion.FinalizoCuadroDialogo {
    ...
    fun pedir_telefono() {
        when {ActivityCompat.checkSelfPermission(activity!!, Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED -> ActivityCompat.requestPermissions(activity!!, arrayOf(Manifest.permission.CALL_PHONE), REQUEST_ACCESS_FINE)
        }
    }
}