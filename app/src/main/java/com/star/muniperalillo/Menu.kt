...
class Menu : FragmentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       // menu_abajo.on(R.id.Noticias, true)
        menu_abajo!!.onItemSelectedListener = {view, item ->
           when(item.itemId){
               2131230729 -> mPager!!.currentItem = 0
               2131230725 -> mPager!!.currentItem = 1
               2131230721 -> mPager!!.currentItem = 2
               2131230726-> mPager!!.currentItem = 3
               2131230730-> mPager!!.currentItem = 4
            }
        }
    ...
    inner class ScreenSlidePagerAdapter(fm: FragmentManager?) :
        ...
        override fun getItem(position: Int): Fragment {
            when (position) {
                0 -> f = NoticiasFragment()
                1 -> f = ConsultasFragment()
                2 -> f = AlertasFragment()
                3 -> f = Covid19Fragment()
                4 -> f = CuentaFragment()
            }
    ...
    inner class PageListener : SimpleOnPageChangeListener() {
        override fun onPageSelected(position: Int) {
            currentPage = position
            when (currentPage){
                0 -> menu_abajo!!.select(2131230729)
                1 -> menu_abajo!!.select(2131230725)
                2 -> menu_abajo!!.select(2131230721)
                3 -> menu_abajo!!.select(2131230726)
                4 -> menu_abajo!!.select(2131230730)
            }
